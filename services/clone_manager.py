...
async def _refresh_clone_channel_view(query, context, db, chat_id, owner_id):
    """Refresh the clone channel settings view without mutating query.data."""
    channel = await db.get_channel(chat_id)
    if not channel:
...
async def clone_callback_handler(update: Update, context):
    ...
    try:
        if data.startswith('verify_force_sub:'):
            ...
            await _refresh_clone_channel_view(query, context, db, chat_id, owner_id)
