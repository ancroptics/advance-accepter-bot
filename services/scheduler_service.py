...
async def _fetch_all_pending_requests_scheduler(application, chat_id):
    """Fetch pending requests via Telethon for scheduler (no context object)."""
    ...
    try:
        ...
        for req in requests:
            result.append({
                'user': {
                    'id': req['user_id'],
                    'first_name': req.get('first_name', 'Unknown'),
                    'username': req.get('username'),
                    'language_code': None,
                },
                'date': int(req['date'].timestamp()) if hasattr(req.get('date'), 'timestamp') else 0,
            })
        logger.info(f'Scheduler Telethon: fetched {len(result)} pending requests for {chat_id}')
        return result
    except Exception as e:
        logger.exception(f'Scheduler error fetching pending requests via Telethon for {chat_id}: {e}')
        return []
        ...
        for ch in channels:
            ...
            try:
                ...
                if telegram_pending > db_pending and telegram_pending > 0:
                    ...
                    try:
                        # Use Telethon MTProto to enumerate ALL pending requests
                        pending_users = await _fetch_all_pending_requests_scheduler(self.application, chat_id)
                        imported = 0
