        ...
        if not channel:
            try:
                ...
                try:
                    ...
                    logger.warning(f'Could not fetch admins for fallback register of {chat_id}: {e}')
                ...
                logger.info(f'Fallback: auto-registered channel {chat_id} from join_request')
                channel = await db.get_channel(chat_id)
            except Exception as e:
                logger.error(f'Fallback channel registration failed for {chat_id}: {e}')
