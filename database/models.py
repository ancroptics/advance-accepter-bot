...
class DatabaseModels:
    ...
    async def get_total_channel_count(self):
        ...
        row = await self.pool.fetchrow("SELECT COUNT(*) as count FROM managed_channels")
    ...
    async def get_platform_stats(self):
        ...
        row = await self.pool.fetchrow("SELECT COUNT(*) as c FROM managed_channels")
        stats['total_channels'] = row['c'] if row else 0
        row = await self.pool.fetchrow("SELECT COUNT(*) as c FROM bot_clones WHERE is_active = true")
    ...
    async def get_all_clones(self):
        """Get all clone bots."""
        rows = await self.pool.fetch("SELECT * FROM bot_clones ORDER BY created_at DESC")
