...
class DatabaseModels:
    ...
    async def get_expired_force_sub_requests(self, chat_id=None, hours=24):
        """Get pending requests with force_sub_required that expired (older than X hours).
        If chat_id is provided, only returns requests for that specific channel."""
        if chat_id:
            return await self.db.fetch("""
                SELECT * FROM join_requests
    ...
    async def get_templates(self, owner_id):
        ...
        try:
            ...
            logger.error(f"get_templates error: {e}")
            return []

    async def create_template(self, owner_id, name, content, content_type='text', media_file_id=None, buttons_json=None):
        """Create a new template."""
        try:
            row = await self.pool.fetchrow(
                "INSERT INTO templates (owner_id, name, content, content_type, media_file_id, buttons_json) "
                "VALUES ($1, $2, $3, $4, $5, $6) RETURNING template_id",
                owner_id, name, content, content_type, media_file_id, buttons_json
            )
    ...
    async def delete_template(self, template_id, owner_id):
        """Delete a template."""
        try:
    ...
    async def save_channel(self, chat_id, chat_title=None, chat_type='channel', owner_id=None, username=None):
        """Alias for channel registration used by handlers.channel_detection."""
        try:
            if owner_id:
                try:
                    await self.upsert_owner(user_id=owner_id)
                except Exception as e:
                    logger.warning(f"save_channel: upsert_owner failed for {owner_id}: {e}")
            return await self.add_channel(
                chat_id=chat_id,
                owner_id=owner_id,
                chat_title=chat_title,
                chat_type=chat_type or 'channel',
                username=username,
            )
        except Exception as e:
            logger.error(f"save_channel error for {chat_id}: {e}")
            raise
