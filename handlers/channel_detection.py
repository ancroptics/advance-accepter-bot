...
async def _handle_bot_added(update, context, chat, db):
    ...
    try:
        ...
        if telethon and telethon.available:
            ...
            # Fallback: check DB for any existing pending requests
            pending_count = await db.get_pending_count(chat.id)
            logger.info(f'Telethon not available, using DB pending count: {pending_count}')
