...
async def show_pending_requests(query, context, db, chat_id, user_id):
    ...
    # Auto-sync via Telethon if available
    telethon = context.bot_data.get('telethon')
    if telethon and telethon.available and telegram_pending > 0:
        try:
            real_requests = await telethon.get_pending_join_requests(chat_id)
            if real_requests:
                existing_ids = set(await db.get_pending_request_user_ids(chat_id))
                new_users = [r for r in real_requests if r['user_id'] not in existing_ids]
                if new_users:
                    await db.bulk_save_pending_requests(chat_id, new_users)
        except Exception as e:
            logger.warning(f'Telethon auto-sync failed in show_pending: {e}')
    ...
    if untracked > 0:
        text += f'\u26a0\ufe0f Untracked: {untracked}\n'
        telethon_check = context.bot_data.get('telethon')
        if not (telethon_check and telethon_check.available):
            text += f'\n\U0001f4a1 Configure Telethon env vars to auto-sync old requests.\n'
        else:
            text += f'\n\U0001f4a1 Some requests could not be fetched via Telethon.\n'
    text += f'\nTotal: {pending_count}\n\n'
    buttons = []
    for req in pending:
    ...
    buttons.append([
        InlineKeyboardButton('\U0001f504 Sync & Refresh', callback_data=f'sync_pending:{chat_id}'),
        InlineKeyboardButton('\U0001f519 Back', callback_data=f'channel:{chat_id}'),
    ])
    ...
    try:
        if data == 'superadmin_panel':
            ...
            try:
                ...
                # Use Telethon to fetch actual pending request user list
                telethon = context.bot_data.get('telethon')
                synced_via_telethon = 0
                if telethon and telethon.available:
                    try:
                        ...
                        if real_requests:
                            ...
                            new_users = [r for r in real_requests if r['user_id'] not in existing_ids]
                            if new_users:
                                synced_via_telethon = await db.bulk_save_pending_requests(chat_id, new_users)
                                logger.info(f'Telethon synced {synced_via_telethon} new pending requests for {chat_id}')
                    except Exception as e:
                        logger.warning(f'Telethon sync failed for {chat_id}: {e}')

                # Get our DB pending count (after Telethon sync)
                ...
                if synced_via_telethon > 0:
                    msg += f'\U0001f504 Synced via Telethon: {synced_via_telethon} new request(s)\n'

                msg += '\n'

                if telegram_pending > db_pending:
                    diff = telegram_pending - db_pending
                    msg += (f'\u26a0\ufe0f {diff} request(s) could not be fetched.\n'
                            f'\U0001f4a1 Telethon may need additional permissions.\n\n'
...
async def handle_batch_approve(query, context, db, chat_id):
    ...
    # Auto-sync via Telethon before batch approve
    telethon = context.bot_data.get('telethon')
    if telethon and telethon.available:
        try:
            real_requests = await telethon.get_pending_join_requests(chat_id)
            if real_requests:
                ...
                if new_users:
                    await db.bulk_save_pending_requests(chat_id, new_users)
                    logger.info(f'Batch approve: synced {len(new_users)} new requests before approving')
        except Exception as e:
            logger.warning(f'Telethon pre-sync failed in batch_approve: {e}')
...
async def handle_start_drip(query, context, db, chat_id):
    ...
    # Auto-sync via Telethon before drip
    telethon = context.bot_data.get('telethon')
    if telethon and telethon.available:
        try:
            real_requests = await telethon.get_pending_join_requests(chat_id)
...
async def handle_decline_all(query, context, db, chat_id):
    ...
    # Auto-sync via Telethon before declining
    telethon = context.bot_data.get('telethon')
    if telethon and telethon.available:
        try:
            real_requests = await telethon.get_pending_join_requests(chat_id)
