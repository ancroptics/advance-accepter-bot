...
def _patch_telegram_login():
    ...
    async def start_telegram_login(update, context):
        ...
        await query.edit_message_text(
            ...
            'Choose a login method. QR login is recommended when Telegram does not deliver a code.',
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton('🔳 Login by QR', callback_data='tg_session_qr_login')],
                [InlineKeyboardButton('☎️ Login by Phone Code', callback_data='tg_session_phone_login')],
                [InlineKeyboardButton('🔙 Back', callback_data='telegram_session_menu')],
            ]),
        )
        return ConversationHandler.END

    async def start_phone_login(update, context):
        query = update.callback_query
        await query.answer()
        await query.edit_message_text(
            '🔑 CONNECT TELEGRAM SESSION\n\n'
    ...
    async def start_qr_login(update, context):
        query = update.callback_query
        await query.answer('Generating QR login...')
        db = context.application.bot_data.get('db')
        user_id = query.from_user.id

        existing = _qr_login_tasks.pop(user_id, None)
        ...
        try:
            ...
            try:
                ...
                pass
            logger.warning(f'QR login start failed for owner {user_id}: {e}')
            await query.edit_message_text(
                f'Could not start QR login: {str(e)[:120]}',
                reply_markup=tg._back_markup(),
            )
            return ConversationHandler.END
        ...
        async def wait_for_qr_scan():
            try:
                ...
                await tg._save_connected_session(db, user_id, encrypt_text(client.session.save()))
                await context.bot.send_message(
                    user_id,
                    '✅ Telegram session connected by QR. You can now sync old pending join requests.',
                    reply_markup=InlineKeyboardMarkup([[
                ...
                await context.bot.send_message(
                    user_id,
                    'QR was scanned, but this Telegram account has 2FA enabled. Use phone-code login so you can enter the 2FA password here.',
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton('☎️ Login by Phone Code', callback_data='tg_session_phone_login')
                    ]]),
                )
                ...
                await context.bot.send_message(
                    user_id,
                    'QR login expired. Start QR login again from Old Request Sync.',
                    reply_markup=tg._back_markup(),
                )
            except Exception as e:
                logger.warning(f'QR login failed for owner {user_id}: {e}')
                await context.bot.send_message(
                    user_id,
                    f'QR login failed: {str(e)[:120]}',
                    reply_markup=tg._back_markup(),
                )
            finally:
        ...
        task = context.application.create_task(wait_for_qr_scan())
        _qr_login_tasks[user_id] = task
        await query.edit_message_text(
            '🔳 QR LOGIN\n\n'
            'Tap the button below from the Telegram app and approve the login request.\n\n'
            'This expires in 2 minutes.',
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton('🔳 Approve QR Login', url=qr_login.url)],
                [InlineKeyboardButton('🔙 Back', callback_data='telegram_session_menu')],
            ]),
        )
        return ConversationHandler.END
    ...
    tg.start_telegram_login = start_telegram_login
    tg._code_markup = _code_markup

    tg.telegram_session_conv_handler = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(start_telegram_login, pattern=r'^tg_session_start$'),
            CallbackQueryHandler(start_phone_login, pattern=r'^tg_session_phone_login$'),
            CallbackQueryHandler(start_qr_login, pattern=r'^tg_session_qr_login$'),
        ],
        states={
